...
const SYNC_BASE_URL = 'http://172.17.0.1:81';
...
class DeltaFile {
  ...
  async consume(callback) {
    ...
    writeStream.on('finish', () => this.ingest(callback));
  ...
  async ingest(callback) {
    ...
    try {
      ...
      await callback(this, true);
      await fs.unlink(this.tmpFilepath);
    } catch (e) {
      await callback(this, false);
