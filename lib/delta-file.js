...
const DOWNLOAD_SHARE_HOST = process.env.DOWNLOAD_SHARE_HOST || SYNC_BASE_URL;
const DOWNLOAD_SHARE_ENDPOINT = `${DOWNLOAD_SHARE_HOST}/fileshare/:id/download`;
    ...
    try {
      ...
      for (let { inserts, deletes } of changeSets) {
        ...
        try {
          ...
          console.error(`Something went wrong while downloading or deleting a file of ${Array.from(shareInserts)} or ${Array.from(shareDeletes)}, but the process continues.`, err);
        }
  ...
  for (let uri of shareURIs) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        throw new Error(`Fetching file failed, response not ok: ${response.status} with error: ${response.statusText}`);
      }
    }
    catch (err) {
